...
class _AdminOrdersScreenState extends State<AdminOrdersScreen> {
            ...
            IconButton(
              ...
              onPressed: () {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Đang kết nối máy in và gửi lệnh in hóa đơn...'), backgroundColor: Colors.blue),
                );
              },
            ),
          ],
          ...
          child: SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                const Center(
                  child: Text('Địa chỉ: 144 Xuân Thủy, Cầu Giấy, Hà Nội\nHotline: 19001008', textAlign: TextAlign.center, style: TextStyle(fontSize: 11)),
                ),
                const Divider(height: 20),
                Text('Mã đơn hàng: #${order.id}', style: const TextStyle(fontWeight: FontWeight.bold)),
                Text('Ngày đặt: ${DateFormat('dd/MM/yyyy HH:mm').format(order.createdAt.toLocal())}'),
                ...
                ...order.items.map((item) => Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Expanded(
                        child: Text('${item.book.title} (x${item.quantity})', maxLines: 1, overflow: TextOverflow.ellipsis),
                      ),
  ...
  void _updateStatus(int orderId, String newStatus, BuildContext bottomSheetCtx) async {
    Navigator.of(bottomSheetCtx).pop(); // close bottomsheet
    final ok = await context.read<OrderProvider>().updateOrderStatus(orderId, newStatus);
    if (ok && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
        ...
        child: orderProv.isLoading && orderProv.adminOrders.isEmpty
            ...
            : RefreshIndicator(
                ...
                child: orderProv.adminOrders.isEmpty
                    ? const Center(child: Text('Chưa có đơn hàng nào từ khách hàng.'))
                    : ListView.builder(
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 6),
                                  Text('Người nhận: ${order.recipientName} - SĐT: ${order.recipientPhone}'),
                                  Text('Địa chỉ: ${order.recipientAddress}'),
                                  Text('Thanh toán: ${order.paymentMethod.toUpperCase()} | Vận chuyển: ${order.deliveryMethod}'),
                                  const Divider(height: 20),
                                  Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          IconButton(
                                            ...
                                            onPressed: () => _changeStatus(order.id, order.status),
                                          ),
                                        ],
                                      ),
                                    ],
